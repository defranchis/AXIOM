...
class test02_scan_iv_map(measurement):
    ...
    def initialise(self):
        ...
        self.lim_cur = 0.000001           # compliance in [A]
        self.lim_vol = 100              # compliance in [V]

        self.cell_list = np.loadtxt('config/channels128.txt', dtype=int)
        self.cellname_list = [511] #np.loadtxt('config/channels128_name.txt', dtype=int)
        self.volt_list = [1]# np.loadtxt('config/voltagesTest.txt', dtype=int)
       ...
       ## Loop over voltages
        try:
            for v in self.volt_list:
                ...
                #pow_supply.ctrl.write(":SOUR:VOLT %f" % v)
            ...
            #pow_supply.ctrl.write(":SOUR:VOLT 0")
