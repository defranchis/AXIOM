...
class testMD_fullSensorMeasurements(measurement):
    ...
    def initialise(self):
        ...
        self.volt_list_iv = [10.-i for i in range(100)]
        ...
        if '_0kGy'in self.id or 'preirr' in self.id:
            ...
            self.volt_list_cv = [0.-i*0.2 for i in range(50)] + [-10.-i*0.5 for i in range(11)]
            self.currents_cv  = [0 for i in self.volt_list_cv]
            self.volt_list_iv = [10.-i for i in range(26)]
            self.currents_iv  = [0 for i in self.volt_list_iv]

        ## might as well get the proper dose
        self.currentDose = int((self.id.split('_')[-1]).replace('kGy','')) 
    ...
    def reset_power_supplies(self):
        ...
        self.keithley2410.ramp_voltage(0)
...
## end of CV scan
    ...
    def execute(self):
        ...
        plots_iv_gcd = self.doIVScan(4, ax2, name='GCD')
