...
def extract_sensor_name(folder_name):
    ...
    #Strip_C_300141_200um_m20C_2024-05-31-01-40-57s
    id = parts[2]
    thickness = parts[3]
    return thickness+" ("+id+")"
        ...
        for dir in dirs:
            if 'm20C' in dir:
                ...
                if sensor_name not in sensor_data:
                    sensor_data[sensor_name] = {'datetime': [], 'capacitance': {}, 'resistance': {}}
                ...
                for subdir, _, subfiles in os.walk(folder_path):
                    ...
                    if cv_file:
                        ...
                        if cv_data is not None:
                            for _, row in cv_data.iterrows():
                                ...
                                if voltage not in sensor_data[sensor_name]['capacitance']:
                                    sensor_data[sensor_name]['capacitance'][voltage] = []
                                sensor_data[sensor_name]['capacitance'][voltage].append((datetime_obj, capacitance))
                    ...
                    for iv_file in iv_files:
                        ...
                        if nominal_voltage is not None and resistance is not None:
                            if nominal_voltage not in sensor_data[sensor_name]['resistance']:
                                sensor_data[sensor_name]['resistance'][nominal_voltage] = []
                            sensor_data[sensor_name]['resistance'][nominal_voltage].append((datetime_obj, resistance, resistance_error))

                sensor_data[sensor_name]['datetime'].append(datetime_obj)

    return sensor_data

def plot_sensor_data(sensor_data, output_dir):
    os.makedirs(output_dir, exist_ok=True)

    for sensor_name, data in sensor_data.items():
        if(data['capacitance'].values()):        
            # Calculate the start date for this sensor
            start_date = min(min(dates for dates, _ in measurements) for measurements in data['capacitance'].values())

            # Plot capacitance
            if data['capacitance']:
                plt.figure(figsize=(12, 6))
                for index, (voltage, measurements) in enumerate(data['capacitance'].items()):
                    if measurements:
                        ...
                        if len(sensor_data.items()) > 1: 
                            color = cmap((index / (len(data['capacitance'].items()) - 1))*0.8+0.1)
                        else:
                            color = cmap(0.5)
                    plt.plot(days, capacitances, label=f'{voltage:.2f}V', color=color, marker='o', markersize=4, linewidth=1.5)
                plt.title(f'Capacitance over time for {sensor_name}', fontweight='bold', fontsize=16)
                ...
                plt.tight_layout()
                plt.savefig(os.path.join(output_dir, f'{sensor_name}_capacitance.png'))
                plt.close()
        else:
            continue

        if(data['resistance'].values()):
            start_date = min(min(dates for dates, _, _ in measurements) for measurements in data['resistance'].values())

            # Plot resistance
            if data['resistance']:
                plt.figure(figsize=(12, 6))
                for index, (voltage, measurements) in enumerate(data['resistance'].items()):
                    if measurements:
                        filtered_measurements = filter_outliers(measurements)
                        ...
                        if len(sensor_data.items()) > 1: 
                            ...
                            color = cmap(0.5)
                        plt.errorbar(days, resistances, yerr=errors, fmt='o-', color=color, label=f'{voltage:.2f}V', marker='o', markersize=4, linewidth=1.5)
                plt.title(f'Resistance over time for {sensor_name}', fontweight='bold', fontsize=16)
                plt.xlabel('Time (Days)', fontweight='bold', fontsize=14)
                ...
                plt.gca().yaxis.set_minor_locator(y_minor)
                plt.tight_layout()
                plt.savefig(os.path.join(output_dir, f'{sensor_name}_resistance.png'))
                plt.close()
        else:
            continue

def save_data_to_npy(sensor_data, output_dir):
    capacitance_data = {}
    resistance_data = {}
    ...
    for sensor_name, data in sensor_data.items():
        if(data['capacitance'].values()):
            ...
            capacitance_data[sensor_name] = {
                voltage: np.array([(date - start_date).total_seconds() / (24 * 3600) for date, _ in measurements] + [cap for _, cap in measurements])
                for voltage, measurements in data['capacitance'].items()
            }
        else:
            continue
        ...
        if(data['resistance'].values()):
            ...
            resistance_data[sensor_name] = {
                voltage: np.array([(date - start_date).total_seconds() / (24 * 3600) for date, _, _ in measurements] + 
                                [res for _, res, _ in measurements] +
                                [err for _, _, err in measurements])
                for voltage, measurements in data['resistance'].items()
            }
        else:
            continue

    np.save(os.path.join(output_dir, 'capacitance_data.npy'), capacitance_data)
    np.save(os.path.join(output_dir, 'resistance_data.npy'), resistance_data)

def main():
    data_dir = 'data/'
    output_dir = 'output/thicknesses/'
    
    sensor_data = analyze_sensor_data(data_dir)
    plot_sensor_data(sensor_data, output_dir)
    save_data_to_npy(sensor_data, output_dir)
